...
document.addEventListener("DOMContentLoaded", function () {
    ...
    categorys.forEach(category => {
        ...
        html += `
                <button id="btn-dm-${category.id}"
                    class="flex items-center justify-center gap-2 text-white text-base sm:text-lg font-medium py-3 px-6 rounded-full mt-4 mb-4 shadow-lg w-full">
                ...
                <div id="dropdown-dm-${category.id}" class="transition-all duration-300 ease-in-out flex flex-col items-center gap-2 w-full">
                    <div class="category-content grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 w-full">
            `;
                ...
                <a href="${product.linkAFF}" class="w-full">
                    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                        ...
                        <div class="p-4">
                            ...
                            <button
                                class="mt-2 bg-black text-white px-4 py-2 rounded-full w-full text-sm sm:text-base">Click
                                me</button>
                        </div>
                    </div>
                </a>
    ...
    const dropdownsOld = [
        { button: "btn-fitness", dropdown: "dropdown-fitness" },
        { button: "btn-decords", dropdown: "dropdown-decords" }
    ];
    ...
    dropdowns.forEach(({ button, dropdown }) => {
        ...
        drop.style.overflow = "hidden";  // Đảm bảo không bị tràn nội dung
        drop.style.transition = "max-height 0.3s ease-in-out"; // Thêm hiệu ứng
        drop.style.maxHeight = "0px"; // Mặc định ẩn
    ...
    searchInput.addEventListener("input", function () {
        ...
        document.querySelectorAll(".category-content div").forEach((product) => {
            const productName = product.querySelector("h3").textContent.toLowerCase();
            if (productName.includes(searchText)) {
                product.style.display = "block"; // Hiển thị nếu có từ khóa
            } else {
